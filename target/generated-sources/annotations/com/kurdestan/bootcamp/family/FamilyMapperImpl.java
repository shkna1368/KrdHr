...
public class FamilyMapperImpl implements FamilyMapper {
    ...
    @Override
    public List<Family> toFamilies(List<FamilyDTO> familyDTOS) {
        if ( familyDTOS == null ) {
            return null;
        }

        List<Family> list = new ArrayList<Family>( familyDTOS.size() );
        for ( FamilyDTO familyDTO : familyDTOS ) {
            list.add( toFamily( familyDTO ) );
        }
